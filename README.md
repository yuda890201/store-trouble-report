...
- 起動時に店舗ごとのアカウントで認証（照合は Firebase Authentication のサーバー側）
...
7. 店舗のアカウントの作成（メールアドレスとパスワードをその場で入力します）
...
**パスワードはスクリプトにもリポジトリにも保存されません。** 入力された内容を
Firebase に送るだけです。
...
Authentication の「ユーザーを追加」から、**店舗ごとに1件ずつ**作成します。

- メールアドレス: 任意（例 `minato@example.com`）。実在しなくても構いません
- パスワード: 6文字以上の任意の文字列（Firebase の要件）

そのアドレスとパスワードが、その店舗のログイン情報になります。

> **メールアドレスもパスワードもコードには書きません。** 画面で入力された内容を
> そのまま Firebase に送り、合っているかどうかの判定は Firebase のサーバー側だけで
> 行われます。一度ログインすればセッションが端末に残るので、毎回の入力は不要です。
